...
RagCode 是一个面向 AI 智能体的**本地代码智能基础设施**，专注于上下文检索。它既不是一个 UI，也不是通用的 RAG 演示，而是一个稳定持久的底座，将结构化代码索引、语义检索、上下文打包与 MCP 集成清晰地分层解耦，让每一层都能独立演进。

它借鉴了 CodeGraph、Understand-Anything 等项目的思路，并在此基础上加入了 LanceDB 语义层以及更强的上下文引擎契约：`get_context` 返回的是当前已索引的、能够帮助智能体回答、调试、修改或评审代码的**最小任务上下文包**——附带明确的来源引用、新鲜度、归属链、拓扑关系，以及一份「还缺少哪些证据」的记录。
