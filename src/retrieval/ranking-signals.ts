...
export function applyRankingSignals(input: RankingSignalInput): RankingSignalResult {
  ...
  if (input.distance) {
    const proximity = graphProximityScore(input.distance.hops) * 1.6;
    const edgeBoost = edgeKindBoost(input.distance.edgeKinds);
    adjustment += proximity + edgeBoost;
    reasons.push(graphReason(input.distance, proximity + edgeBoost));
