...
export const javaAnalyzer: LanguageAnalyzer = {
  ...
  analyzeFile: ({ repoRoot, file, content }) => analyzeJavaFile(repoRoot, file, content)
};

export function analyzeJavaFile(repoRoot: string, file: CodeFile, content: string): FileAnalysis {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    const source = /^\s*import\s+([^;]+);/.exec(lines[index]!)?.[1];
    if (!source) continue;
    edges.push({
      ...
      kind: "imports",
      metadata: { source, sourceFile: file.path, line: index + 1, resolution: "java_import" }
    });
  }
  ...
  for (const declaration of declarationsFor(lines)) {
    ...
    if (symbol.exported) {
      edges.push({ projectId: file.projectId, sourceId: fileSymbol.id, targetId: symbol.id, kind: "exports", metadata: { sourceFile: file.path, name: symbol.name, resolution: "java_export" } });
    }
    for (const call of callsIn(lines.slice(symbol.startLine - 1, symbol.endLine).join("\n"))) {
      edges.push({
        ...
        kind: "calls",
        metadata: { targetName: call.name, sourceFile: file.path, line: symbol.startLine + call.lineOffset, resolution: "java_call" }
      });
    }
  }
...
function declarationsFor(lines: string[]): Declaration[] {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const classMatch = /^\s*(public\s+)?(?:abstract\s+|final\s+)?(?:class|interface|enum)\s+([A-Za-z_]\w*)\b/.exec(line);
    const methodMatch = /^\s*(public|protected|private)?\s*(?:static\s+)?(?:final\s+)?[A-Za-z_<>\[\], ?]+\s+([A-Za-z_]\w*)\s*\([^;]*\)\s*\{?/.exec(line);
    const name = classMatch?.[2] ?? methodMatch?.[2];
    if (!name || ["if", "for", "while", "switch", "catch"].includes(name)) continue;
    declarations.push({
      name,
      kind: classMatch ? "class" : "method",
      startLine: index + 1,
      endLine: blockEnd(lines, index),
      signature: line.trim(),
      exported: classMatch ? Boolean(classMatch[1]) : methodMatch?.[1] === "public"
    });
  }
  return declarations;
}
