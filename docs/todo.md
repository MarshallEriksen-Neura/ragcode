...
## F. 发布后 owner 排名质量优化 (2026-06-11，非当前发布阻断)
...
暂不作为本次发布阻断：AI 使用 `context` / `search` 时已有足够证据自行判断目标是否是测试文件；真实用户反馈和后续 benchmark 覆盖更能校准 owner 直出权重，避免发布前只针对单个案例过拟合。

- [ ] F1 owner-specific benchmark: 直接覆盖 `engine.findOwner` / `ragcode owner`，不要只用 `getContext().ownerChain` 代替；新增 "production owner + test fake/mock 大量符号刷分" 负例。
- [ ] F2 多语言测试路径降权: 复用 / 扩展 `classifyEvidencePath`，覆盖 JS/TS `*.test.*` / `*.spec.*` / `__tests__` / `__mocks__`，Python `test_*.py` / `*_test.py`，Go `*_test.go`，Java/Kotlin `src/test` / `*Test.*`，Rust `tests/`，Ruby `*_spec.rb`，PHP `*Test.php`，C# `*Tests.cs`，以及 fixtures/mocks/examples/sample/generated 等通用路径。
- [ ] F3 intent-aware ranking: 普通 owner 查询中 implementation 优先、test/fixture/docs 降权；明确 test intent 查询中允许测试文件成为 primary；没有 implementation 命中时 test/fixture 可作为 fallback，但结果理由要标注 supporting/test evidence。
- [ ] F4 owner 输出质量门槛: 增加断言 production owner 排名前于 test/mock fixture；同时保留一个明确 test 查询回归，避免把测试定位能力修坏。
