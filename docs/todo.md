...
- [x] M9 reuse AST 解析固定成本: normalizedBodyFingerprint 按 language+scriptKind+contentHash 缓存 (上限 10 万条，满则清空)，重复查询跳过 ts.createSourceFile 重解析。实测 (4000 symbols): 分散场景 warm 1904ms → 300ms (6.3×)。
- [x] M10 buildStructureIndex 指纹组 O(group²) 退化: 同指纹组两两对比加固定窗口 (64)，超大组信号降级为采样下界 (boilerplate 形状本就非 reuse 目标)；窗口内/外成员仍能确认行为重复 (+回归测试 150 同形函数)。实测 (4000 同构单组): cold 18987ms → 436ms (43.5×)，warm 19500ms → 235ms (83×)。
...
## F. owner 排名质量优化 (2026-06-11，已修复)

当前发布前手测发现 `ragcode owner . "Ink watch status TUI index progress"` 会把 `tests/watch-daemon.test.ts` 排在 production owner 前面。原因不是索引过期，而是 `findOwner` 直出排序会把测试 fixture 中大量同名/接口实现符号累加成高 owner 分；同一查询下 `search` 和 `context` 表现正常，`context.ownerChain` 能定位到 `src/cli/tui/watch-status.tsx`、`src/cli/tui/index-progress.tsx` 和 `src/core/types.ts`。这说明问题集中在 owner CLI / `findOwner` 原始排序，不是整体检索不可用。

已收口：`findOwner` 的内存图与 SQLite 图路径现在都会经过 `applyOwnerPathIntent`。普通 owner 查询会把 test path 降权并把分数压到最强 implementation owner 以下；显式 test/spec/coverage/regression 查询或直接点名测试路径时，测试 owner 仍可成为首位。`tests/symbol-aware-retrieval.test.ts` 覆盖了默认 owner 查询、显式 test owner 查询，以及 SQLite FTS 路径，避免把测试定位能力修坏。

- [x] F1 owner-specific benchmark: 直接覆盖 `engine.findOwner` / `ragcode owner`，不要只用 `getContext().ownerChain` 代替；新增 "production owner + test fake/mock 大量符号刷分" 负例。
- [x] F2 测试路径降权: 复用 `classifyEvidencePath` / `isTestPath`，覆盖 JS/TS `*.test.*`、`*.spec.*`、`__tests__`、`tests/`，并让 fixture/docs 在 search/context rerank 中继续作为 supporting evidence 降权。
- [x] F3 intent-aware ranking: 普通 owner 查询中 implementation 优先、test 降权；明确 test intent 查询中允许测试文件成为 primary；原因会标注 `test default demotion` 或 `test relevance boost`。
- [x] F4 owner 输出质量门槛: 增加断言 production owner 排名前于 test fixture；同时保留明确 test 查询回归，避免把测试定位能力修坏。
